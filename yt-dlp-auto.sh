...
function scrape-download() {
    ...
    for url in "${YouTubeURL[@]}"; do
        ...
        yt-dlp -f "bestvideo[height=1080]+bestaudio/best[height=1080]" --output "$video_dir/%(title)s.%(ext)s" "$url" >>"$video_dir/$YouTubeTitle.log" 2>&1 &
        # Alternatively, download audio only (uncomment if needed)
        # yt-dlp -f "bestvideo[height=1080]+bestaudio/best[height=1080]" --extract-audio --audio-format mp3 --output "$audio_dir/%(title)s.%(ext)s" "$url" & >> "$audio_dir/$YouTubeTitle.log" 2>&1 &
    done
